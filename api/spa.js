  ...
  try {
    ...
    if (resource === 'therapists') {
      ...
      if (req.method === 'DELETE' && id) {
        const used = await sql`SELECT COUNT(*)::int AS n FROM appointments WHERE therapist_id = ${id} AND status IN ('pending','confirmed','inProgress')`;
        if (used[0].n > 0) return res.status(400).json({ error: 'Therapist has active appointments.' });
    ...
    if (resource === 'offers') {
      ...
      if (req.method === 'POST') {
        ...
        const rows = await sql`
          ...
          VALUES (${name}, ${code||null}, ${type||'pct'}, ${value}, ${min_amount||0},
    ...
    if (resource === 'appointments') {
      ...
      if (req.method === 'POST') {
        const { customer_id, customer_name, customer_phone, customer_email, therapist_id, room_id,
          ...
          payment_method, notes, staff_id } = req.body || {};
        ...
        const rows = await sql`
          INSERT INTO appointments (customer_id, customer_name, customer_phone, customer_email,
            ...
            paid_amount, payment_method, notes, staff_id)
      ...
      if (req.method === 'PUT' && id) {
        ...
        if (add_payment !== undefined) {
          rows = await sql`UPDATE appointments SET paid_amount = LEAST(total_amount, paid_amount + ${Number(add_payment)})${payment_method ? sql`, payment_method = ${payment_method}` : sql``} WHERE id = ${id} RETURNING *`;
        } else if (status === 'cancelled') {
          rows = await sql`UPDATE appointments SET status = 'cancelled', total_amount = paid_amount WHERE id = ${id} RETURNING *`;
        } else {
          rows = await sql`UPDATE appointments SET status = COALESCE(${status??null}, status), paid_amount = COALESCE(${paid_amount??null}, paid_amount) WHERE id = ${id} RETURNING *`;
        }
        if (!rows?.length) return res.status(404).json({ error: 'Not found' });
        return res.status(200).json(rows[0]);
      }
      if (req.method === 'DELETE' && id) {
        const check = await sql`SELECT status FROM appointments WHERE id = ${id}`;
        if (!check.length) return res.status(404).json({ error: 'Not found' });
        if (check[0].status !== 'cancelled') return res.status(400).json({ error: 'Only cancelled appointments can be deleted' });
    ...
    if (resource === 'reception') {
      if (req.method === 'GET') {
        ...
        const rows = date
          ? await sql`SELECT r.*, t.name AS therapist_name, rm.name AS room_name FROM reception_log r LEFT JOIN therapists t ON t.id=r.therapist_id LEFT JOIN rooms rm ON rm.id=r.room_id WHERE r.in_time::date = ${date} ORDER BY r.in_time DESC`
      ...
      if (req.method === 'POST') {
        const { customer_name, customer_phone, customer_email, customer_id, therapist_id, room_id,
          ...
          paid_amount, payment_method, notes, staff_id } = req.body || {};
        ...
        const rows = await sql`
          INSERT INTO reception_log (customer_name, customer_phone, customer_email, customer_id,
            ...
            total_amount, paid_amount, payment_method, notes, staff_id)
    ...
    if (resource === 'staff') {
      ...
      if (req.method === 'PUT' && id) {
        ...
        const rows = pin
          ? await sql`UPDATE staff SET name=COALESCE(${name??null},name),phone=COALESCE(${phone??null},phone),role=COALESCE(${role??null},role),pin_hash=${pin},active=COALESCE(${active??null},active) WHERE id=${id} RETURNING id,name,email,phone,role,active`
    ...
    if (resource === 'customers') {
      if (req.method === 'POST' && action === 'register') {
        ...
        if (!name || !email || !password || password.length < 6) return res.status(400).json({ error: 'name, email and password (min 6) required' });
    ...
    if (resource === 'expenses') {
      if (req.method === 'GET') {
        const rows = await sql`SELECT * FROM expenses ORDER BY expense_date DESC`;
        return res.status(200).json(rows);
      }
      if (req.method === 'POST') {
    ...
    console.error('spa API error:', err.message);
